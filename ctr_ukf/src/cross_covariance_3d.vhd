...
architecture Behavioral of cross_covariance_3d is
    ...
    signal chi_x_pos, chi_x_vel, chi_x_omega : state_array := (others => (others => '0'));
    signal chi_y_pos, chi_y_vel, chi_y_omega : state_array := (others => (others => '0'));
    signal chi_z_pos, chi_z_vel, chi_z_omega : state_array := (others => (others => '0'));
    ...
    signal x_pos_mean_reg, x_vel_mean_reg, x_omega_mean_reg : signed(47 downto 0);
    signal y_pos_mean_reg, y_vel_mean_reg, y_omega_mean_reg : signed(47 downto 0);
    signal z_pos_mean_reg, z_vel_mean_reg, z_omega_mean_reg : signed(47 downto 0);
        ...
        if rising_edge(clk) then
            case state is
                ...
                when IDLE =>
                    ...
                    if start = '1' then
                        report "CROSS_COV: IDLE->COMPUTE_DELTAS (latching inputs)" & LF &
                               "  chi0_x_pos = " & integer'image(to_integer(chi0_x_pos)) & LF &
                               "  chi1_x_pos = " & integer'image(to_integer(chi1_x_pos)) & LF &
                               "  x_pos_mean = " & integer'image(to_integer(x_pos_mean)) & LF &
                               "  z_x_mean = " & integer'image(to_integer(z_x_mean));

                        x_pos_mean_reg <= x_pos_mean; x_vel_mean_reg <= x_vel_mean; x_omega_mean_reg <= x_omega_mean;
                        y_pos_mean_reg <= y_pos_mean; y_vel_mean_reg <= y_vel_mean; y_omega_mean_reg <= y_omega_mean;
                        z_pos_mean_reg <= z_pos_mean; z_vel_mean_reg <= z_vel_mean; z_omega_mean_reg <= z_omega_mean;
                        z_x_mean_reg <= z_x_mean; z_y_mean_reg <= z_y_mean; z_z_mean_reg <= z_z_mean;

                        chi_x_pos(0) <= chi0_x_pos; chi_x_vel(0) <= chi0_x_vel; chi_x_omega(0) <= chi0_x_omega;
                        chi_y_pos(0) <= chi0_y_pos; chi_y_vel(0) <= chi0_y_vel; chi_y_omega(0) <= chi0_y_omega;
                        chi_z_pos(0) <= chi0_z_pos; chi_z_vel(0) <= chi0_z_vel; chi_z_omega(0) <= chi0_z_omega;

                        chi_x_pos(1) <= chi1_x_pos; chi_x_vel(1) <= chi1_x_vel; chi_x_omega(1) <= chi1_x_omega;
                        chi_y_pos(1) <= chi1_y_pos; chi_y_vel(1) <= chi1_y_vel; chi_y_omega(1) <= chi1_y_omega;
                        chi_z_pos(1) <= chi1_z_pos; chi_z_vel(1) <= chi1_z_vel; chi_z_omega(1) <= chi1_z_omega;

                        chi_x_pos(2) <= chi2_x_pos; chi_x_vel(2) <= chi2_x_vel; chi_x_omega(2) <= chi2_x_omega;
                        chi_y_pos(2) <= chi2_y_pos; chi_y_vel(2) <= chi2_y_vel; chi_y_omega(2) <= chi2_y_omega;
                        chi_z_pos(2) <= chi2_z_pos; chi_z_vel(2) <= chi2_z_vel; chi_z_omega(2) <= chi2_z_omega;

                        chi_x_pos(3) <= chi3_x_pos; chi_x_vel(3) <= chi3_x_vel; chi_x_omega(3) <= chi3_x_omega;
                        chi_y_pos(3) <= chi3_y_pos; chi_y_vel(3) <= chi3_y_vel; chi_y_omega(3) <= chi3_y_omega;
                        chi_z_pos(3) <= chi3_z_pos; chi_z_vel(3) <= chi3_z_vel; chi_z_omega(3) <= chi3_z_omega;

                        chi_x_pos(4) <= chi4_x_pos; chi_x_vel(4) <= chi4_x_vel; chi_x_omega(4) <= chi4_x_omega;
                        chi_y_pos(4) <= chi4_y_pos; chi_y_vel(4) <= chi4_y_vel; chi_y_omega(4) <= chi4_y_omega;
                        chi_z_pos(4) <= chi4_z_pos; chi_z_vel(4) <= chi4_z_vel; chi_z_omega(4) <= chi4_z_omega;

                        chi_x_pos(5) <= chi5_x_pos; chi_x_vel(5) <= chi5_x_vel; chi_x_omega(5) <= chi5_x_omega;
                        chi_y_pos(5) <= chi5_y_pos; chi_y_vel(5) <= chi5_y_vel; chi_y_omega(5) <= chi5_y_omega;
                        chi_z_pos(5) <= chi5_z_pos; chi_z_vel(5) <= chi5_z_vel; chi_z_omega(5) <= chi5_z_omega;

                        chi_x_pos(6) <= chi6_x_pos; chi_x_vel(6) <= chi6_x_vel; chi_x_omega(6) <= chi6_x_omega;
                        chi_y_pos(6) <= chi6_y_pos; chi_y_vel(6) <= chi6_y_vel; chi_y_omega(6) <= chi6_y_omega;
                        chi_z_pos(6) <= chi6_z_pos; chi_z_vel(6) <= chi6_z_vel; chi_z_omega(6) <= chi6_z_omega;

                        chi_x_pos(7) <= chi7_x_pos; chi_x_vel(7) <= chi7_x_vel; chi_x_omega(7) <= chi7_x_omega;
                        chi_y_pos(7) <= chi7_y_pos; chi_y_vel(7) <= chi7_y_vel; chi_y_omega(7) <= chi7_y_omega;
                        chi_z_pos(7) <= chi7_z_pos; chi_z_vel(7) <= chi7_z_vel; chi_z_omega(7) <= chi7_z_omega;

                        chi_x_pos(8) <= chi8_x_pos; chi_x_vel(8) <= chi8_x_vel; chi_x_omega(8) <= chi8_x_omega;
                        chi_y_pos(8) <= chi8_y_pos; chi_y_vel(8) <= chi8_y_vel; chi_y_omega(8) <= chi8_y_omega;
                        chi_z_pos(8) <= chi8_z_pos; chi_z_vel(8) <= chi8_z_vel; chi_z_omega(8) <= chi8_z_omega;

                        chi_x_pos(9) <= chi9_x_pos; chi_x_vel(9) <= chi9_x_vel; chi_x_omega(9) <= chi9_x_omega;
                        chi_y_pos(9) <= chi9_y_pos; chi_y_vel(9) <= chi9_y_vel; chi_y_omega(9) <= chi9_y_omega;
                        chi_z_pos(9) <= chi9_z_pos; chi_z_vel(9) <= chi9_z_vel; chi_z_omega(9) <= chi9_z_omega;

                        chi_x_pos(10) <= chi10_x_pos; chi_x_vel(10) <= chi10_x_vel; chi_x_omega(10) <= chi10_x_omega;
                        chi_y_pos(10) <= chi10_y_pos; chi_y_vel(10) <= chi10_y_vel; chi_y_omega(10) <= chi10_y_omega;
                        chi_z_pos(10) <= chi10_z_pos; chi_z_vel(10) <= chi10_z_vel; chi_z_omega(10) <= chi10_z_omega;

                        chi_x_pos(11) <= chi11_x_pos; chi_x_vel(11) <= chi11_x_vel; chi_x_omega(11) <= chi11_x_omega;
                        chi_y_pos(11) <= chi11_y_pos; chi_y_vel(11) <= chi11_y_vel; chi_y_omega(11) <= chi11_y_omega;
                        chi_z_pos(11) <= chi11_z_pos; chi_z_vel(11) <= chi11_z_vel; chi_z_omega(11) <= chi11_z_omega;

                        chi_x_pos(12) <= chi12_x_pos; chi_x_vel(12) <= chi12_x_vel; chi_x_omega(12) <= chi12_x_omega;
                        chi_y_pos(12) <= chi12_y_pos; chi_y_vel(12) <= chi12_y_vel; chi_y_omega(12) <= chi12_y_omega;
                        chi_z_pos(12) <= chi12_z_pos; chi_z_vel(12) <= chi12_z_vel; chi_z_omega(12) <= chi12_z_omega;

                        chi_x_pos(13) <= chi13_x_pos; chi_x_vel(13) <= chi13_x_vel; chi_x_omega(13) <= chi13_x_omega;
                        chi_y_pos(13) <= chi13_y_pos; chi_y_vel(13) <= chi13_y_vel; chi_y_omega(13) <= chi13_y_omega;
                        chi_z_pos(13) <= chi13_z_pos; chi_z_vel(13) <= chi13_z_vel; chi_z_omega(13) <= chi13_z_omega;

                        chi_x_pos(14) <= chi14_x_pos; chi_x_vel(14) <= chi14_x_vel; chi_x_omega(14) <= chi14_x_omega;
                        chi_y_pos(14) <= chi14_y_pos; chi_y_vel(14) <= chi14_y_vel; chi_y_omega(14) <= chi14_y_omega;
                        chi_z_pos(14) <= chi14_z_pos; chi_z_vel(14) <= chi14_z_vel; chi_z_omega(14) <= chi14_z_omega;

                        chi_x_pos(15) <= chi15_x_pos; chi_x_vel(15) <= chi15_x_vel; chi_x_omega(15) <= chi15_x_omega;
                        chi_y_pos(15) <= chi15_y_pos; chi_y_vel(15) <= chi15_y_vel; chi_y_omega(15) <= chi15_y_omega;
                        chi_z_pos(15) <= chi15_z_pos; chi_z_vel(15) <= chi15_z_vel; chi_z_omega(15) <= chi15_z_omega;

                        chi_x_pos(16) <= chi16_x_pos; chi_x_vel(16) <= chi16_x_vel; chi_x_omega(16) <= chi16_x_omega;
                        chi_y_pos(16) <= chi16_y_pos; chi_y_vel(16) <= chi16_y_vel; chi_y_omega(16) <= chi16_y_omega;
                        chi_z_pos(16) <= chi16_z_pos; chi_z_vel(16) <= chi16_z_vel; chi_z_omega(16) <= chi16_z_omega;

                        chi_x_pos(17) <= chi17_x_pos; chi_x_vel(17) <= chi17_x_vel; chi_x_omega(17) <= chi17_x_omega;
                        chi_y_pos(17) <= chi17_y_pos; chi_y_vel(17) <= chi17_y_vel; chi_y_omega(17) <= chi17_y_omega;
                        chi_z_pos(17) <= chi17_z_pos; chi_z_vel(17) <= chi17_z_vel; chi_z_omega(17) <= chi17_z_omega;

                        chi_x_pos(18) <= chi18_x_pos; chi_x_vel(18) <= chi18_x_vel; chi_x_omega(18) <= chi18_x_omega;
                        chi_y_pos(18) <= chi18_y_pos; chi_y_vel(18) <= chi18_y_vel; chi_y_omega(18) <= chi18_y_omega;
                        chi_z_pos(18) <= chi18_z_pos; chi_z_vel(18) <= chi18_z_vel; chi_z_omega(18) <= chi18_z_omega;
                ...
                when COMPUTE_DELTAS =>
                    report "COV_RECON: COMPUTE_DELTA idx=" & integer'image(accumulate_idx) & LF &
                           ...
                           "  chi_z_pos(" & integer'image(accumulate_idx) & ")=" & integer'image(to_integer(chi_z_pos(accumulate_idx))) & LF &
                           "  z_pos_mean=" & integer'image(to_integer(z_pos_mean_reg));
                ...
                when COMPUTE_OUTER =>
                    ...
                    if accumulate_idx = 7 then
                        report "CROSS_COV: OUTER_PRODUCT idx=7 (Z-axis sigma point)" & LF &
                               "  delta_z_pos=" & integer'image(to_integer(delta_z_pos)) & LF &
                               "  delta_z_x=" & integer'image(to_integer(delta_z_x)) & LF &
                               "  delta_z_z=" & integer'image(to_integer(delta_z_z));
                    end if;
                    ...
                    outer_31 <= delta_x_omega * delta_z_x; outer_32 <= delta_x_omega * delta_z_y; outer_33 <= delta_x_omega * delta_z_z;
                    ...
                    outer_61 <= delta_y_omega * delta_z_x; outer_62 <= delta_y_omega * delta_z_y; outer_63 <= delta_y_omega * delta_z_z;
                    ...
                    outer_91 <= delta_z_omega * delta_z_x; outer_92 <= delta_z_omega * delta_z_y; outer_93 <= delta_z_omega * delta_z_z;
                ...
                when FINISHED =>
                    report "CROSS_COV: FINISHED (Pxz matrix, Q24.24)" & LF &
                           "  Column 1 (X-meas): pxz_11=" & integer'image(to_integer(pxz_11)) &
                           ", pxz_71=" & integer'image(to_integer(pxz_71)) & LF &
                           "  Column 3 (Z-meas): pxz_13=" & integer'image(to_integer(pxz_13)) &
                           " (" & real'image(real(to_integer(pxz_13)) / 16777216.0) & ")" & LF &
                           "    pxz_73=" & integer'image(to_integer(pxz_73)) &
                           " (" & real'image(real(to_integer(pxz_73)) / 16777216.0) & ")" & LF &
                           "    pxz_83=" & integer'image(to_integer(pxz_83)) &
                           " (" & real'image(real(to_integer(pxz_83)) / 16777216.0) & ")" & LF &
                           "    pxz_93=" & integer'image(to_integer(pxz_93)) &
                           " (" & real'image(real(to_integer(pxz_93)) / 16777216.0) & ")";
                    done <= '1';
                    if start = '0' then
                        state <= IDLE;
                    end if;
