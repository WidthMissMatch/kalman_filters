...
entity predicted_mean_3d is
  port (
    ...
    chi0_x_pos_pred, chi0_x_vel_pred, chi0_x_omega_pred : in signed(47 downto 0);
    chi0_y_pos_pred, chi0_y_vel_pred, chi0_y_omega_pred : in signed(47 downto 0);
    chi0_z_pos_pred, chi0_z_vel_pred, chi0_z_omega_pred : in signed(47 downto 0);
...
architecture Behavioral of predicted_mean_3d is
  ...
  signal x_pos_mean_int, x_vel_mean_int, x_omega_mean_int : signed(47 downto 0) := (others => '0');
  signal y_pos_mean_int, y_vel_mean_int, y_omega_mean_int : signed(47 downto 0) := (others => '0');
  signal z_pos_mean_int, z_vel_mean_int, z_omega_mean_int : signed(47 downto 0) := (others => '0');
...
begin
  ...
  begin
    if rst = '1' then
      ...
      x_pos_mean_int <= (others => '0'); x_vel_mean_int <= (others => '0'); x_omega_mean_int <= (others => '0');
      y_pos_mean_int <= (others => '0'); y_vel_mean_int <= (others => '0'); y_omega_mean_int <= (others => '0');
      z_pos_mean_int <= (others => '0'); z_vel_mean_int <= (others => '0'); z_omega_mean_int <= (others => '0');
      ...
      case state is
        ...
        when MULTIPLY =>
          report "PREDICTED_MEAN_3D: MULTIPLY state (19 points x 9 states)" & LF &
          ...
          w_chi0_xa <= W0 * chi0_x_omega_pred;
          w_chi1_xa <= W1 * chi1_x_omega_pred; w_chi2_xa <= W1 * chi2_x_omega_pred; w_chi3_xa <= W1 * chi3_x_omega_pred; w_chi4_xa <= W1 * chi4_x_omega_pred; w_chi5_xa <= W1 * chi5_x_omega_pred;
          w_chi6_xa <= W1 * chi6_x_omega_pred; w_chi7_xa <= W1 * chi7_x_omega_pred; w_chi8_xa <= W1 * chi8_x_omega_pred; w_chi9_xa <= W1 * chi9_x_omega_pred;
          w_chi10_xa <= W1 * chi10_x_omega_pred; w_chi11_xa <= W1 * chi11_x_omega_pred; w_chi12_xa <= W1 * chi12_x_omega_pred; w_chi13_xa <= W1 * chi13_x_omega_pred; w_chi14_xa <= W1 * chi14_x_omega_pred;
          w_chi15_xa <= W1 * chi15_x_omega_pred; w_chi16_xa <= W1 * chi16_x_omega_pred; w_chi17_xa <= W1 * chi17_x_omega_pred; w_chi18_xa <= W1 * chi18_x_omega_pred;
          ...
          w_chi0_ya <= W0 * chi0_y_omega_pred;
          w_chi1_ya <= W1 * chi1_y_omega_pred; w_chi2_ya <= W1 * chi2_y_omega_pred; w_chi3_ya <= W1 * chi3_y_omega_pred; w_chi4_ya <= W1 * chi4_y_omega_pred; w_chi5_ya <= W1 * chi5_y_omega_pred;
          w_chi6_ya <= W1 * chi6_y_omega_pred; w_chi7_ya <= W1 * chi7_y_omega_pred; w_chi8_ya <= W1 * chi8_y_omega_pred; w_chi9_ya <= W1 * chi9_y_omega_pred;
          w_chi10_ya <= W1 * chi10_y_omega_pred; w_chi11_ya <= W1 * chi11_y_omega_pred; w_chi12_ya <= W1 * chi12_y_omega_pred; w_chi13_ya <= W1 * chi13_y_omega_pred; w_chi14_ya <= W1 * chi14_y_omega_pred;
          w_chi15_ya <= W1 * chi15_y_omega_pred; w_chi16_ya <= W1 * chi16_y_omega_pred; w_chi17_ya <= W1 * chi17_y_omega_pred; w_chi18_ya <= W1 * chi18_y_omega_pred;
          ...
          w_chi0_za <= W0 * chi0_z_omega_pred;
          w_chi1_za <= W1 * chi1_z_omega_pred; w_chi2_za <= W1 * chi2_z_omega_pred; w_chi3_za <= W1 * chi3_z_omega_pred; w_chi4_za <= W1 * chi4_z_omega_pred; w_chi5_za <= W1 * chi5_z_omega_pred;
          w_chi6_za <= W1 * chi6_z_omega_pred; w_chi7_za <= W1 * chi7_z_omega_pred; w_chi8_za <= W1 * chi8_z_omega_pred; w_chi9_za <= W1 * chi9_z_omega_pred;
          w_chi10_za <= W1 * chi10_z_omega_pred; w_chi11_za <= W1 * chi11_z_omega_pred; w_chi12_za <= W1 * chi12_z_omega_pred; w_chi13_za <= W1 * chi13_z_omega_pred; w_chi14_za <= W1 * chi14_z_omega_pred;
          w_chi15_za <= W1 * chi15_z_omega_pred; w_chi16_za <= W1 * chi16_z_omega_pred; w_chi17_za <= W1 * chi17_z_omega_pred; w_chi18_za <= W1 * chi18_z_omega_pred;

          state <= ACCUMULATE;

        when ACCUMULATE =>
          report "PREDICTED_MEAN_3D: ACCUMULATE state (19 weighted products per state)";
        ...
        when OUTPUT =>
          ...
          x_omega_mean_int <= resize(sum_xa, 48);
          y_omega_mean_int <= resize(sum_ya, 48);
          z_omega_mean_int <= resize(sum_za, 48);

          state <= FINISHED;

        when FINISHED =>
          report "PREDICTED_MEAN_3D: FINISHED (done='1')" & LF &
                 "  x_pos_mean_pred = " & integer'image(to_integer(x_pos_mean_pred)) & LF &
                 "  x_vel_mean_pred = " & integer'image(to_integer(x_vel_mean_pred)) & LF &
                 "  y_pos_mean_pred = " & integer'image(to_integer(y_pos_mean_pred));
          done <= '1';
          if start = '0' then
            state <= IDLE;
          end if;
