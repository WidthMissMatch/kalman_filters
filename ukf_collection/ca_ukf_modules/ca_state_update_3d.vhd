        ...
        if rising_edge(clk) then
            case state is
                ...
                when IDLE =>
                    ...
                    if start = '1' then
                        report "STATE_UPDATE: IDLE->UPDATE_STATE (latching inputs)" & LF &
                               "  x_pos_pred = " & integer'image(to_integer(x_pos_pred)) & LF &
                               "  k11 = " & integer'image(to_integer(k11)) & LF &
                               "  k21 = " & integer'image(to_integer(k21)) & LF &
                               "  nu_x = " & integer'image(to_integer(nu_x)) & LF &
                               "  nu_y = " & integer'image(to_integer(nu_y)) & LF &
                               "  nu_z = " & integer'image(to_integer(nu_z)) & LF &
                               "  p11_pred = " & integer'image(to_integer(p11_pred));
                ...
                when UPDATE_STATE =>
                    ...
                    report "STATE_UPDATE: UPDATE_STATE (applying Kalman correction)";
                ...
                when SHIFT_AP =>
                    ...
                    report "STATE_UPDATE: SHIFT_AP" & LF &
                           "  ap(0)(95:48) before shift = " & integer'image(to_integer(ap(0)(95 downto 48)));
                ...
                when ADD_APAT_KRK =>
                    ...
                    report "STATE_UPDATE: ADD_APAT_KRK (computing final covariance)" & LF &
                           "  apat_11(95:48) = " & integer'image(to_integer(apat_11(95 downto 48))) & LF &
                           "  krk_11(95:48) = " & integer'image(to_integer(krk_11(95 downto 48))) & LF &
                           "  sum(95:48) = " & integer'image(to_integer(temp_sum(95 downto 48)));
                    ...
                    report "STATE_UPDATE: P comparison (before/after measurement update)" & LF &
                           "  p11_pred = " & integer'image(to_integer(p11_pred)) &
                           ", p11_upd = " & integer'image(to_integer(p11_upd)) & LF &
                           "  p77_pred = " & integer'image(to_integer(p77_pred)) &
                           ", p77_upd = " & integer'image(to_integer(p77_upd)) & LF &
                           "  p99_pred = " & integer'image(to_integer(p99_pred)) &
                           ", p99_upd = " & integer'image(to_integer(p99_upd));

                    state <= FINISHED;
                when FINISHED =>
                    report "STATE_UPDATE: FINISHED (done='1')" & LF &
                           ...
                           "  p11_upd = " & integer'image(to_integer(p11_upd));
                    done <= '1';
                    if start = '0' then
                        state <= IDLE;
                    end if;
