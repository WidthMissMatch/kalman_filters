...
architecture Behavioral of kalman_gain_3d is
    ...
    begin
        ...
        if shifted > UNITY then
            report "WARNING: Kalman gain saturated to +1.0 (" &
                   integer'image(to_integer(shifted)) & " > " &
                   integer'image(to_integer(UNITY)) & ")" severity warning;
            return UNITY;
        elsif shifted < NEG_UNITY then
            report "WARNING: Kalman gain saturated to -1.0 (" &
                   integer'image(to_integer(shifted)) & " < " &
                   integer'image(to_integer(NEG_UNITY)) & ")" severity warning;
            return NEG_UNITY;
        else
            return shifted;
        end if;
    ...
    begin
        if rising_edge(clk) then
            case state is
                ...
                when FINISHED =>
                    report "KALMAN_GAIN: FINISHED (K matrix, Q24.24)" & LF &
                           "  Column 1 (X-meas): k11=" & integer'image(to_integer(k11)) &
                           " (" & real'image(real(to_integer(k11)) / 16777216.0) & ")" &
                           ", k71=" & integer'image(to_integer(k71)) &
                           " (" & real'image(real(to_integer(k71)) / 16777216.0) & ")" & LF &
                           "  Column 3 (Z-meas): k13=" & integer'image(to_integer(k13)) &
                           " (" & real'image(real(to_integer(k13)) / 16777216.0) & ")" & LF &
                           "    k73=" & integer'image(to_integer(k73)) &
                           " (" & real'image(real(to_integer(k73)) / 16777216.0) & ")" & LF &
                           "    k83=" & integer'image(to_integer(k83)) &
                           " (" & real'image(real(to_integer(k83)) / 16777216.0) & ")" & LF &
                           "    k93=" & integer'image(to_integer(k93)) &
                           " (" & real'image(real(to_integer(k93)) / 16777216.0) & ")";
                    done <= '1';
                    if start = '0' then
                        state <= IDLE;
                    end if;
