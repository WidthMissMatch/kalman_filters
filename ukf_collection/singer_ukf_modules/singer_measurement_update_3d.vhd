    ...
    begin
        if rising_edge(clk) then
            case state is
                ...
                when FINISHED =>
                    report "MEASUREMENT_UPDATE: FINISHED state" & LF &
                           "  x_pos_upd (from state_update) = " & integer'image(to_integer(x_pos_upd)) & LF &
                           "  y_pos_upd (from state_update) = " & integer'image(to_integer(y_pos_upd)) & LF &
                           "  z_pos_upd (from state_update) = " & integer'image(to_integer(z_pos_upd)) & LF &
                           "  These are passed through to ukf_supreme_3d";
                    done <= '1';
                    if start = '0' then
                        report "MEASUREMENT_UPDATE: Transitioning FINISHED -> IDLE";
                        state <= IDLE;
                    end if;
