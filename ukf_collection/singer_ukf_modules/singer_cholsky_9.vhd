...
architecture Behavioral of cholesky_9x9 is
  ...
  component cholesky_col2_parallel is
    ...
    port (
      ...
      l32, l42, l52, l62, l72, l82, l92 : buffer signed(47 downto 0);
      done : out std_logic
    );
  end component;
  ...
  component cholesky_col4_parallel is
    ...
    port (
      ...
      l54, l64, l74, l84, l94 : buffer signed(47 downto 0);
      done : out std_logic
    );
  end component;
  ...
  component cholesky_col5_parallel is
    ...
    port (
      ...
      l65, l75, l85, l95 : buffer signed(47 downto 0);
      done : out std_logic
    );
  end component;
  ...
  begin
    if rising_edge(clk) then
      ...
      case state is
        when IDLE =>
          if start = '1' then
            report "CHOLESKY_9x9: IDLE - Latching inputs, p99_in=" & integer'image(to_integer(p99_in));
        ...
        when WAIT_L99 =>
          if sqrt_l99_done = '1' then
            l99 <= sqrt_l99_output;
            report "CHOLESKY_9x9: L99 computed = " & integer'image(to_integer(sqrt_l99_output));
            state <= FINISHED;
          end if;
