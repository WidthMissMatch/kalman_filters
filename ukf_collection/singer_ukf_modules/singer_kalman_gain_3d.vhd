...
architecture Behavioral of kalman_gain_3d is
    ...
    constant GAIN_MAX : signed(47 downto 0) := to_signed(134217728, 48);
    constant GAIN_MIN : signed(47 downto 0) := to_signed(-134217728, 48);
    ...
    begin
        ...
        if shifted > GAIN_MAX then
            report "WARNING: Kalman gain saturated to +8.0 (" &
                   integer'image(to_integer(shifted)) & " > " &
                   integer'image(to_integer(GAIN_MAX)) & ")" severity warning;
            return GAIN_MAX;
        elsif shifted < GAIN_MIN then
            report "WARNING: Kalman gain saturated to -8.0 (" &
                   integer'image(to_integer(shifted)) & " < " &
                   integer'image(to_integer(GAIN_MIN)) & ")" severity warning;
            return GAIN_MIN;
        else
            return shifted;
        end if;
        ...
        if rising_edge(clk) then
            case state is
                ...
                when IDLE =>
                    ...
                    if start = '1' then
                        report "KALMAN_GAIN: IDLE->INVERT_S (latching inputs)" & LF &
                               "  pxz_11 = " & integer'image(to_integer(pxz_11)) & LF &
                               "  pxz_21 = " & integer'image(to_integer(pxz_21)) & LF &
                               "  s11 = " & integer'image(to_integer(s11)) & LF &
                               "  s22 = " & integer'image(to_integer(s22)) & LF &
                ...
                when LATCH_S_INV =>
                    ...
                    report "KALMAN_GAIN: LATCH_S_INV (matrix inverse latched)" & LF &
                           "  s_inv_11 = " & integer'image(to_integer(s_inv_11)) & LF &
                           "  s_inv_22 = " & integer'image(to_integer(s_inv_22)) & LF &
                           "  s_inv_33 = " & integer'image(to_integer(s_inv_33));
                ...
                when NORMALIZE =>
                    ...
                    report "KALMAN_GAIN: NORMALIZE (converting to Q24.24 format with saturation)";
                ...
                when FINISHED =>
                    report "KALMAN_GAIN: FINISHED (done='1')" & LF &
                           "  k11 = " & integer'image(to_integer(k11)) & LF &
                           "  k21 = " & integer'image(to_integer(k21)) & LF &
                           "  k41 = " & integer'image(to_integer(k41)) & LF &
                           "  k42 = " & integer'image(to_integer(k42)) & LF &
                           "  k52 = " & integer'image(to_integer(k52)) & LF &
                           "  k71 = " & integer'image(to_integer(k71)) & LF &
                           "  k73 = " & integer'image(to_integer(k73)) & LF &
                           "  k83 = " & integer'image(to_integer(k83));
                    done <= '1';
                    if start = '0' then
                        state <= IDLE;
                    end if;
