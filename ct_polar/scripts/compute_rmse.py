#!/usr/bin/env python3
"""
Compute RMSE for CT Polar UKF VHDL output vs ground truth
...
def main():
    ...
    # Format: cycle,px_hex,py_hex,v_hex,theta_hex,omega_hex,a_hex,z_hex,vz_hex,az_hex,p11,...,p99
    vhdl_lines = []
    with open(vhdl_file, 'r') as f:
        ...
        for line in f:
            ...
            if len(parts) >= 10:
                ...
                vz = hex48_to_float(parts[8])
                az = hex48_to_float(parts[9])
                vhdl_lines.append({
    ...
    print(f"=== CT Polar UKF RMSE Results ===")
    ...
    print(f"  vz range: [{last10['vz'].min():.4f}, {last10['vz'].max():.4f}] m/s")
    print(f"  az range: [{last10['az'].min():.6f}, {last10['az'].max():.6f}] m/s²")
