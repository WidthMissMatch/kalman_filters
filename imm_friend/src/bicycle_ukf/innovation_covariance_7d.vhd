...
architecture Behavioral of innovation_covariance_7d is
    ...
    signal acc_s11, acc_s12, acc_s22 : signed(95 downto 0) := (others => '0');
    signal acc_s13, acc_s23, acc_s33 : signed(95 downto 0) := (others => '0');
        ...
        if rising_edge(clk) then
            case state is
                ...
                when ADD =>
                    ...
                    acc_s11 <= acc_s11 + resize(shift_right(outer_11 * current_weight, 2*Q), 96);
                    acc_s12 <= acc_s12 + resize(shift_right(outer_12 * current_weight, 2*Q), 96);
                    acc_s22 <= acc_s22 + resize(shift_right(outer_22 * current_weight, 2*Q), 96);
                    acc_s13 <= acc_s13 + resize(shift_right(outer_13 * current_weight, 2*Q), 96);
                    acc_s23 <= acc_s23 + resize(shift_right(outer_23 * current_weight, 2*Q), 96);
                    acc_s33 <= acc_s33 + resize(shift_right(outer_33 * current_weight, 2*Q), 96);
                ...
                when ADD_R =>
                    acc_s11 <= acc_s11 + resize(R11_Q24, 96);
                    acc_s22 <= acc_s22 + resize(R22_Q24, 96);
                    acc_s33 <= acc_s33 + resize(R33_Q24, 96);
                    state <= SATURATE;

                when SATURATE =>
                    if acc_s11 > resize(MAX_VALUE, 96) then s11 <= MAX_VALUE;
                    elsif acc_s11 < 0 then s11 <= to_signed(0, 48);
                    else s11 <= resize(acc_s11, 48); end if;

                    if acc_s22 > resize(MAX_VALUE, 96) then s22 <= MAX_VALUE;
                    elsif acc_s22 < 0 then s22 <= to_signed(0, 48);
                    else s22 <= resize(acc_s22, 48); end if;

                    if acc_s33 > resize(MAX_VALUE, 96) then s33 <= MAX_VALUE;
                    elsif acc_s33 < 0 then s33 <= to_signed(0, 48);
                    else s33 <= resize(acc_s33, 48); end if;

                    if acc_s12 > resize(MAX_VALUE, 96) then s12 <= MAX_VALUE;
                    elsif acc_s12 < resize(MIN_VALUE, 96) then s12 <= MIN_VALUE;
                    else s12 <= resize(acc_s12, 48); end if;

                    if acc_s13 > resize(MAX_VALUE, 96) then s13 <= MAX_VALUE;
                    elsif acc_s13 < resize(MIN_VALUE, 96) then s13 <= MIN_VALUE;
                    else s13 <= resize(acc_s13, 48); end if;

                    if acc_s23 > resize(MAX_VALUE, 96) then s23 <= MAX_VALUE;
                    elsif acc_s23 < resize(MIN_VALUE, 96) then s23 <= MIN_VALUE;
                    else s23 <= resize(acc_s23, 48); end if;
