...
entity ctra_ukf_supreme_imm is
  port (
    clk   : in  std_logic;
    reset : in  std_logic;
    start : in  std_logic;
...
architecture Behavioral of ctra_ukf_supreme_imm is
  ...
  constant MAX_P : signed(47 downto 0) := to_signed(2147483647, 48);
  constant MIN_P_DIAG : signed(47 downto 0) := to_signed(168, 48);

  function sat_p(val : signed(47 downto 0)) return signed is
  begin
    if val > MAX_P then return MAX_P;
    elsif val < -MAX_P then return -MAX_P;
    else return val;
    end if;
  end function;

  function sat_p_diag(val : signed(47 downto 0)) return signed is
  begin
    if val > MAX_P then return MAX_P;
    elsif val < MIN_P_DIAG then return MIN_P_DIAG;
    else return val;
    end if;
  end function;
  ...
  constant P_INIT_5 : signed(47 downto 0) := to_signed(8388608, 48);
...
begin
  ...
  prop_inst : predicti_ctra
    port map (
      ...
      chi0_px_in => sig0_px, chi0_py_in => sig0_py, chi0_v_in => sig0_v, chi0_theta_in => sig0_th, chi0_omega_in => sig0_dl, chi0_a_in => sig0_ac, chi0_z_in => sig0_z,
      chi1_px_in => sig1_px, chi1_py_in => sig1_py, chi1_v_in => sig1_v, chi1_theta_in => sig1_th, chi1_omega_in => sig1_dl, chi1_a_in => sig1_ac, chi1_z_in => sig1_z,
      chi2_px_in => sig2_px, chi2_py_in => sig2_py, chi2_v_in => sig2_v, chi2_theta_in => sig2_th, chi2_omega_in => sig2_dl, chi2_a_in => sig2_ac, chi2_z_in => sig2_z,
      chi3_px_in => sig3_px, chi3_py_in => sig3_py, chi3_v_in => sig3_v, chi3_theta_in => sig3_th, chi3_omega_in => sig3_dl, chi3_a_in => sig3_ac, chi3_z_in => sig3_z,
      chi4_px_in => sig4_px, chi4_py_in => sig4_py, chi4_v_in => sig4_v, chi4_theta_in => sig4_th, chi4_omega_in => sig4_dl, chi4_a_in => sig4_ac, chi4_z_in => sig4_z,
      chi5_px_in => sig5_px, chi5_py_in => sig5_py, chi5_v_in => sig5_v, chi5_theta_in => sig5_th, chi5_omega_in => sig5_dl, chi5_a_in => sig5_ac, chi5_z_in => sig5_z,
      chi6_px_in => sig6_px, chi6_py_in => sig6_py, chi6_v_in => sig6_v, chi6_theta_in => sig6_th, chi6_omega_in => sig6_dl, chi6_a_in => sig6_ac, chi6_z_in => sig6_z,
      chi7_px_in => sig7_px, chi7_py_in => sig7_py, chi7_v_in => sig7_v, chi7_theta_in => sig7_th, chi7_omega_in => sig7_dl, chi7_a_in => sig7_ac, chi7_z_in => sig7_z,
      chi8_px_in => sig8_px, chi8_py_in => sig8_py, chi8_v_in => sig8_v, chi8_theta_in => sig8_th, chi8_omega_in => sig8_dl, chi8_a_in => sig8_ac, chi8_z_in => sig8_z,
      chi9_px_in => sig9_px, chi9_py_in => sig9_py, chi9_v_in => sig9_v, chi9_theta_in => sig9_th, chi9_omega_in => sig9_dl, chi9_a_in => sig9_ac, chi9_z_in => sig9_z,
      chi10_px_in => sig10_px, chi10_py_in => sig10_py, chi10_v_in => sig10_v, chi10_theta_in => sig10_th, chi10_omega_in => sig10_dl, chi10_a_in => sig10_ac, chi10_z_in => sig10_z,
      chi11_px_in => sig11_px, chi11_py_in => sig11_py, chi11_v_in => sig11_v, chi11_theta_in => sig11_th, chi11_omega_in => sig11_dl, chi11_a_in => sig11_ac, chi11_z_in => sig11_z,
      chi12_px_in => sig12_px, chi12_py_in => sig12_py, chi12_v_in => sig12_v, chi12_theta_in => sig12_th, chi12_omega_in => sig12_dl, chi12_a_in => sig12_ac, chi12_z_in => sig12_z,
      chi13_px_in => sig13_px, chi13_py_in => sig13_py, chi13_v_in => sig13_v, chi13_theta_in => sig13_th, chi13_omega_in => sig13_dl, chi13_a_in => sig13_ac, chi13_z_in => sig13_z,
      chi14_px_in => sig14_px, chi14_py_in => sig14_py, chi14_v_in => sig14_v, chi14_theta_in => sig14_th, chi14_omega_in => sig14_dl, chi14_a_in => sig14_ac, chi14_z_in => sig14_z,
      chi0_px_out => pr0_px, chi0_py_out => pr0_py, chi0_v_out => pr0_v, chi0_theta_out => pr0_th, chi0_omega_out => pr0_dl, chi0_a_out => pr0_ac, chi0_z_out => pr0_z,
      chi1_px_out => pr1_px, chi1_py_out => pr1_py, chi1_v_out => pr1_v, chi1_theta_out => pr1_th, chi1_omega_out => pr1_dl, chi1_a_out => pr1_ac, chi1_z_out => pr1_z,
      chi2_px_out => pr2_px, chi2_py_out => pr2_py, chi2_v_out => pr2_v, chi2_theta_out => pr2_th, chi2_omega_out => pr2_dl, chi2_a_out => pr2_ac, chi2_z_out => pr2_z,
      chi3_px_out => pr3_px, chi3_py_out => pr3_py, chi3_v_out => pr3_v, chi3_theta_out => pr3_th, chi3_omega_out => pr3_dl, chi3_a_out => pr3_ac, chi3_z_out => pr3_z,
      chi4_px_out => pr4_px, chi4_py_out => pr4_py, chi4_v_out => pr4_v, chi4_theta_out => pr4_th, chi4_omega_out => pr4_dl, chi4_a_out => pr4_ac, chi4_z_out => pr4_z,
      chi5_px_out => pr5_px, chi5_py_out => pr5_py, chi5_v_out => pr5_v, chi5_theta_out => pr5_th, chi5_omega_out => pr5_dl, chi5_a_out => pr5_ac, chi5_z_out => pr5_z,
      chi6_px_out => pr6_px, chi6_py_out => pr6_py, chi6_v_out => pr6_v, chi6_theta_out => pr6_th, chi6_omega_out => pr6_dl, chi6_a_out => pr6_ac, chi6_z_out => pr6_z,
      chi7_px_out => pr7_px, chi7_py_out => pr7_py, chi7_v_out => pr7_v, chi7_theta_out => pr7_th, chi7_omega_out => pr7_dl, chi7_a_out => pr7_ac, chi7_z_out => pr7_z,
      chi8_px_out => pr8_px, chi8_py_out => pr8_py, chi8_v_out => pr8_v, chi8_theta_out => pr8_th, chi8_omega_out => pr8_dl, chi8_a_out => pr8_ac, chi8_z_out => pr8_z,
      chi9_px_out => pr9_px, chi9_py_out => pr9_py, chi9_v_out => pr9_v, chi9_theta_out => pr9_th, chi9_omega_out => pr9_dl, chi9_a_out => pr9_ac, chi9_z_out => pr9_z,
      chi10_px_out => pr10_px, chi10_py_out => pr10_py, chi10_v_out => pr10_v, chi10_theta_out => pr10_th, chi10_omega_out => pr10_dl, chi10_a_out => pr10_ac, chi10_z_out => pr10_z,
      chi11_px_out => pr11_px, chi11_py_out => pr11_py, chi11_v_out => pr11_v, chi11_theta_out => pr11_th, chi11_omega_out => pr11_dl, chi11_a_out => pr11_ac, chi11_z_out => pr11_z,
      chi12_px_out => pr12_px, chi12_py_out => pr12_py, chi12_v_out => pr12_v, chi12_theta_out => pr12_th, chi12_omega_out => pr12_dl, chi12_a_out => pr12_ac, chi12_z_out => pr12_z,
      chi13_px_out => pr13_px, chi13_py_out => pr13_py, chi13_v_out => pr13_v, chi13_theta_out => pr13_th, chi13_omega_out => pr13_dl, chi13_a_out => pr13_ac, chi13_z_out => pr13_z,
      chi14_px_out => pr14_px, chi14_py_out => pr14_py, chi14_v_out => pr14_v, chi14_theta_out => pr14_th, chi14_omega_out => pr14_dl, chi14_a_out => pr14_ac, chi14_z_out => pr14_z,
      done => prop_done
    );
        ...
        case fsm_state is
          ...
          when IDLE =>
            ...
            if start = '1' then
              ...
              if first_cycle = '1' or inject_en = '1' then
          ...
          when INIT_STATE =>
            if first_cycle = '1' then
              ...
              if inject_state_only_latched = '0' then
                cp11 <= inj_p11; cp12 <= (others => '0'); cp13 <= (others => '0');
                cp14 <= (others => '0'); cp15 <= (others => '0'); cp16 <= (others => '0'); cp17 <= (others => '0');
                cp22 <= inj_p22; cp23 <= (others => '0'); cp24 <= (others => '0');
                cp25 <= (others => '0'); cp26 <= (others => '0'); cp27 <= (others => '0');
                cp33 <= inj_p33; cp34 <= (others => '0'); cp35 <= (others => '0');
                cp36 <= (others => '0'); cp37 <= (others => '0');
                cp44 <= inj_p44; cp45 <= (others => '0'); cp46 <= (others => '0'); cp47 <= (others => '0');
                cp55 <= inj_p55; cp56 <= (others => '0'); cp57 <= (others => '0');
                cp66 <= inj_p66; cp67 <= (others => '0');
                cp77 <= inj_p77;
              end if;
          ...
          when WAIT_PROC_NOISE =>
            if pn_done = '1' then
              ...
              ppred11 <= sat_p_diag(pn11); ppred12 <= sat_p(pp12_s); ppred13 <= sat_p(pp13_s);
              ppred14 <= sat_p(pp14_s); ppred15 <= sat_p(pp15_s); ppred16 <= sat_p(pp16_s); ppred17 <= sat_p(pp17_s);
              ppred22 <= sat_p_diag(pn22); ppred23 <= sat_p(pp23_s); ppred24 <= sat_p(pp24_s);
              ppred25 <= sat_p(pp25_s); ppred26 <= sat_p(pp26_s); ppred27 <= sat_p(pp27_s);
              ppred33 <= sat_p_diag(pn33); ppred34 <= sat_p(pp34_s); ppred35 <= sat_p(pp35_s);
              ppred36 <= sat_p(pp36_s); ppred37 <= sat_p(pp37_s);
              ppred44 <= sat_p_diag(pn44); ppred45 <= sat_p(pp45_s); ppred46 <= sat_p(pp46_s); ppred47 <= sat_p(pp47_s);
              ppred55 <= sat_p_diag(pn55); ppred56 <= sat_p(pp56_s); ppred57 <= sat_p(pp57_s);
              ppred66 <= sat_p_diag(pn66); ppred67 <= sat_p(pp67_s);
              ppred77 <= sat_p_diag(pn77);
          ...
          when LATCH_OUTPUT =>
            ...
            t_state <= wrap_angle(xu4); w_state <= xu5; a_state <= xu6; z_state <= xu7;

            cp11 <= sat_p_diag(pu11); cp12 <= sat_p(pu12); cp13 <= sat_p(pu13); cp14 <= sat_p(pu14);
            cp15 <= sat_p(pu15); cp16 <= sat_p(pu16); cp17 <= sat_p(pu17);
            cp22 <= sat_p_diag(pu22); cp23 <= sat_p(pu23); cp24 <= sat_p(pu24);
            cp25 <= sat_p(pu25); cp26 <= sat_p(pu26); cp27 <= sat_p(pu27);
            cp33 <= sat_p_diag(pu33); cp34 <= sat_p(pu34); cp35 <= sat_p(pu35);
            cp36 <= sat_p(pu36); cp37 <= sat_p(pu37);
            cp44 <= sat_p_diag(pu44); cp45 <= sat_p(pu45); cp46 <= sat_p(pu46); cp47 <= sat_p(pu47);
            cp55 <= sat_p_diag(pu55); cp56 <= sat_p(pu56); cp57 <= sat_p(pu57);
            cp66 <= sat_p_diag(pu66); cp67 <= sat_p(pu67);
            cp77 <= sat_p_diag(pu77);

            px_current <= xu1; py_current <= xu2; v_current <= xu3;
            theta_current <= xu4; omega_current <= xu5; a_current <= xu6; z_current <= xu7;
            p11_diag <= sat_p_diag(pu11); p22_diag <= sat_p_diag(pu22); p33_diag <= sat_p_diag(pu33);
            p44_diag <= sat_p_diag(pu44); p55_diag <= sat_p_diag(pu55); p66_diag <= sat_p_diag(pu66); p77_diag <= sat_p_diag(pu77);

            nu_x_sig <= nu1; nu_y_sig <= nu2; nu_z_sig <= nu3;
            s11_sig <= ss11; s22_sig <= ss22; s33_sig <= ss33;
