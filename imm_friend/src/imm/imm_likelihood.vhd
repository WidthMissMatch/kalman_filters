...
architecture Behavioral of imm_likelihood is
  ...
  type state_type is (IDLE, SANITIZE_S, COMPUTE_NU_SQ, COMPUTE_S_RECIP, APPLY_S_INV,
  ...
  constant S_MIN : signed(47 downto 0) := to_signed(16777, 48);

  constant LOGL_PENALTY : signed(47 downto 0) := to_signed(-167772160, 48);

  signal ss1_11, ss1_22, ss1_33 : signed(47 downto 0) := (others => '0');
  signal ss2_11, ss2_22, ss2_33 : signed(47 downto 0) := (others => '0');
  signal ss3_11, ss3_22, ss3_33 : signed(47 downto 0) := (others => '0');

  signal model1_bad, model2_bad, model3_bad : std_logic := '0';
  ...
  begin
    if rising_edge(clk) then
      case state is
        ...
        when SANITIZE_S =>
          model1_bad <= '0'; model2_bad <= '0'; model3_bad <= '0';

          if s1_11 <= S_MIN then ss1_11 <= S_MIN; model1_bad <= '1';
          else ss1_11 <= s1_11; end if;
          if s1_22 <= S_MIN then ss1_22 <= S_MIN; model1_bad <= '1';
          else ss1_22 <= s1_22; end if;
          if s1_33 <= S_MIN then ss1_33 <= S_MIN;
          else ss1_33 <= s1_33; end if;

          if s2_11 <= S_MIN then ss2_11 <= S_MIN; model2_bad <= '1';
          else ss2_11 <= s2_11; end if;
          if s2_22 <= S_MIN then ss2_22 <= S_MIN; model2_bad <= '1';
          else ss2_22 <= s2_22; end if;
          if s2_33 <= S_MIN then ss2_33 <= S_MIN;
          else ss2_33 <= s2_33; end if;

          if s3_11 <= S_MIN then ss3_11 <= S_MIN; model3_bad <= '1';
          else ss3_11 <= s3_11; end if;
          if s3_22 <= S_MIN then ss3_22 <= S_MIN; model3_bad <= '1';
          else ss3_22 <= s3_22; end if;
          if s3_33 <= S_MIN then ss3_33 <= S_MIN;
          else ss3_33 <= s3_33; end if;
        ...
        when COMPUTE_NU_SQ =>
          ...
          rs1_11 <= nr_initial_guess(ss1_11);
          rs1_22 <= nr_initial_guess(ss1_22);
          rs1_33 <= nr_initial_guess(ss1_33);
          rs2_11 <= nr_initial_guess(ss2_11);
          rs2_22 <= nr_initial_guess(ss2_22);
          rs2_33 <= nr_initial_guess(ss2_33);
          rs3_11 <= nr_initial_guess(ss3_11);
          rs3_22 <= nr_initial_guess(ss3_22);
          rs3_33 <= nr_initial_guess(ss3_33);
          nr_iter <= 0;
