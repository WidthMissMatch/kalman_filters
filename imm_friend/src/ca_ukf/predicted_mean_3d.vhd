  ...
  begin
    if rst = '1' then
      ...
      case state is
        ...
        when MULTIPLY =>
          report "PREDICTED_MEAN_3D: MULTIPLY state (19 points x 9 states, values hex-suppressed)";
        ...
        when OUTPUT =>
          report "PREDICTED_MEAN_3D: OUTPUT state (values hex-suppressed)";
        ...
        when FINISHED =>
          report "PREDICTED_MEAN_3D: FINISHED (done='1', values hex-suppressed)";
          done <= '1';
          if start = '0' then
            state <= IDLE;
          end if;
