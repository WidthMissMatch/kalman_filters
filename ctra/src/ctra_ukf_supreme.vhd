...
architecture Behavioral of ctra_ukf_supreme is
  ...
  component predicted_mean_7d is
    port (
      ...
      mean_s1, mean_s2, mean_s3, mean_s4, mean_s5, mean_s6, mean_s7 : buffer signed(47 downto 0);
      done : out std_logic
    );
  end component;
  ...
  component process_noise_ctra is
    port (
      clk, rst, start : in std_logic;
  ...
  signal sig0_px, sig0_py, sig0_v, sig0_th, sig0_om, sig0_ac, sig0_z : signed(47 downto 0);
  signal sig1_px, sig1_py, sig1_v, sig1_th, sig1_om, sig1_ac, sig1_z : signed(47 downto 0);
  signal sig2_px, sig2_py, sig2_v, sig2_th, sig2_om, sig2_ac, sig2_z : signed(47 downto 0);
  signal sig3_px, sig3_py, sig3_v, sig3_th, sig3_om, sig3_ac, sig3_z : signed(47 downto 0);
  signal sig4_px, sig4_py, sig4_v, sig4_th, sig4_om, sig4_ac, sig4_z : signed(47 downto 0);
  signal sig5_px, sig5_py, sig5_v, sig5_th, sig5_om, sig5_ac, sig5_z : signed(47 downto 0);
  signal sig6_px, sig6_py, sig6_v, sig6_th, sig6_om, sig6_ac, sig6_z : signed(47 downto 0);
  signal sig7_px, sig7_py, sig7_v, sig7_th, sig7_om, sig7_ac, sig7_z : signed(47 downto 0);
  signal sig8_px, sig8_py, sig8_v, sig8_th, sig8_om, sig8_ac, sig8_z : signed(47 downto 0);
  signal sig9_px, sig9_py, sig9_v, sig9_th, sig9_om, sig9_ac, sig9_z : signed(47 downto 0);
  signal sig10_px, sig10_py, sig10_v, sig10_th, sig10_om, sig10_ac, sig10_z : signed(47 downto 0);
  signal sig11_px, sig11_py, sig11_v, sig11_th, sig11_om, sig11_ac, sig11_z : signed(47 downto 0);
  signal sig12_px, sig12_py, sig12_v, sig12_th, sig12_om, sig12_ac, sig12_z : signed(47 downto 0);
  signal sig13_px, sig13_py, sig13_v, sig13_th, sig13_om, sig13_ac, sig13_z : signed(47 downto 0);
  signal sig14_px, sig14_py, sig14_v, sig14_th, sig14_om, sig14_ac, sig14_z : signed(47 downto 0);

  signal prop_start, prop_done : std_logic;
  signal pr0_px, pr0_py, pr0_v, pr0_th, pr0_om, pr0_ac, pr0_z : signed(47 downto 0);
  signal pr1_px, pr1_py, pr1_v, pr1_th, pr1_om, pr1_ac, pr1_z : signed(47 downto 0);
  signal pr2_px, pr2_py, pr2_v, pr2_th, pr2_om, pr2_ac, pr2_z : signed(47 downto 0);
  signal pr3_px, pr3_py, pr3_v, pr3_th, pr3_om, pr3_ac, pr3_z : signed(47 downto 0);
  signal pr4_px, pr4_py, pr4_v, pr4_th, pr4_om, pr4_ac, pr4_z : signed(47 downto 0);
  signal pr5_px, pr5_py, pr5_v, pr5_th, pr5_om, pr5_ac, pr5_z : signed(47 downto 0);
  signal pr6_px, pr6_py, pr6_v, pr6_th, pr6_om, pr6_ac, pr6_z : signed(47 downto 0);
  signal pr7_px, pr7_py, pr7_v, pr7_th, pr7_om, pr7_ac, pr7_z : signed(47 downto 0);
  signal pr8_px, pr8_py, pr8_v, pr8_th, pr8_om, pr8_ac, pr8_z : signed(47 downto 0);
  signal pr9_px, pr9_py, pr9_v, pr9_th, pr9_om, pr9_ac, pr9_z : signed(47 downto 0);
  signal pr10_px, pr10_py, pr10_v, pr10_th, pr10_om, pr10_ac, pr10_z : signed(47 downto 0);
  signal pr11_px, pr11_py, pr11_v, pr11_th, pr11_om, pr11_ac, pr11_z : signed(47 downto 0);
  signal pr12_px, pr12_py, pr12_v, pr12_th, pr12_om, pr12_ac, pr12_z : signed(47 downto 0);
  signal pr13_px, pr13_py, pr13_v, pr13_th, pr13_om, pr13_ac, pr13_z : signed(47 downto 0);
  signal pr14_px, pr14_py, pr14_v, pr14_th, pr14_om, pr14_ac, pr14_z : signed(47 downto 0);
...
begin
  ...
  sigma_inst : sigma_7d
    port map (
      ...
      theta_mean => t_state, omega_mean => w_state, a_mean => a_state, z_mean => z_state,
      ...
      chi0_px => sig0_px, chi0_py => sig0_py, chi0_v => sig0_v, chi0_theta => sig0_th, chi0_omega => sig0_om, chi0_a => sig0_ac, chi0_z => sig0_z,
      chi1_px => sig1_px, chi1_py => sig1_py, chi1_v => sig1_v, chi1_theta => sig1_th, chi1_omega => sig1_om, chi1_a => sig1_ac, chi1_z => sig1_z,
      chi2_px => sig2_px, chi2_py => sig2_py, chi2_v => sig2_v, chi2_theta => sig2_th, chi2_omega => sig2_om, chi2_a => sig2_ac, chi2_z => sig2_z,
      chi3_px => sig3_px, chi3_py => sig3_py, chi3_v => sig3_v, chi3_theta => sig3_th, chi3_omega => sig3_om, chi3_a => sig3_ac, chi3_z => sig3_z,
      chi4_px => sig4_px, chi4_py => sig4_py, chi4_v => sig4_v, chi4_theta => sig4_th, chi4_omega => sig4_om, chi4_a => sig4_ac, chi4_z => sig4_z,
      chi5_px => sig5_px, chi5_py => sig5_py, chi5_v => sig5_v, chi5_theta => sig5_th, chi5_omega => sig5_om, chi5_a => sig5_ac, chi5_z => sig5_z,
      chi6_px => sig6_px, chi6_py => sig6_py, chi6_v => sig6_v, chi6_theta => sig6_th, chi6_omega => sig6_om, chi6_a => sig6_ac, chi6_z => sig6_z,
      chi7_px => sig7_px, chi7_py => sig7_py, chi7_v => sig7_v, chi7_theta => sig7_th, chi7_omega => sig7_om, chi7_a => sig7_ac, chi7_z => sig7_z,
      chi8_px => sig8_px, chi8_py => sig8_py, chi8_v => sig8_v, chi8_theta => sig8_th, chi8_omega => sig8_om, chi8_a => sig8_ac, chi8_z => sig8_z,
      chi9_px => sig9_px, chi9_py => sig9_py, chi9_v => sig9_v, chi9_theta => sig9_th, chi9_omega => sig9_om, chi9_a => sig9_ac, chi9_z => sig9_z,
      chi10_px => sig10_px, chi10_py => sig10_py, chi10_v => sig10_v, chi10_theta => sig10_th, chi10_omega => sig10_om, chi10_a => sig10_ac, chi10_z => sig10_z,
      chi11_px => sig11_px, chi11_py => sig11_py, chi11_v => sig11_v, chi11_theta => sig11_th, chi11_omega => sig11_om, chi11_a => sig11_ac, chi11_z => sig11_z,
      chi12_px => sig12_px, chi12_py => sig12_py, chi12_v => sig12_v, chi12_theta => sig12_th, chi12_omega => sig12_om, chi12_a => sig12_ac, chi12_z => sig12_z,
      chi13_px => sig13_px, chi13_py => sig13_py, chi13_v => sig13_v, chi13_theta => sig13_th, chi13_omega => sig13_om, chi13_a => sig13_ac, chi13_z => sig13_z,
      chi14_px => sig14_px, chi14_py => sig14_py, chi14_v => sig14_v, chi14_theta => sig14_th, chi14_omega => sig14_om, chi14_a => sig14_ac, chi14_z => sig14_z,
      done => sigma_done
    );

  prop_inst : predicti_ctra
    port map (
      clk => clk, rst => reset, start => prop_start,
      chi0_px_in => sig0_px, chi0_py_in => sig0_py, chi0_v_in => sig0_v, chi0_theta_in => sig0_th, chi0_omega_in => sig0_om, chi0_a_in => sig0_ac, chi0_z_in => sig0_z,
      chi1_px_in => sig1_px, chi1_py_in => sig1_py, chi1_v_in => sig1_v, chi1_theta_in => sig1_th, chi1_omega_in => sig1_om, chi1_a_in => sig1_ac, chi1_z_in => sig1_z,
      chi2_px_in => sig2_px, chi2_py_in => sig2_py, chi2_v_in => sig2_v, chi2_theta_in => sig2_th, chi2_omega_in => sig2_om, chi2_a_in => sig2_ac, chi2_z_in => sig2_z,
      chi3_px_in => sig3_px, chi3_py_in => sig3_py, chi3_v_in => sig3_v, chi3_theta_in => sig3_th, chi3_omega_in => sig3_om, chi3_a_in => sig3_ac, chi3_z_in => sig3_z,
      chi4_px_in => sig4_px, chi4_py_in => sig4_py, chi4_v_in => sig4_v, chi4_theta_in => sig4_th, chi4_omega_in => sig4_om, chi4_a_in => sig4_ac, chi4_z_in => sig4_z,
      chi5_px_in => sig5_px, chi5_py_in => sig5_py, chi5_v_in => sig5_v, chi5_theta_in => sig5_th, chi5_omega_in => sig5_om, chi5_a_in => sig5_ac, chi5_z_in => sig5_z,
      chi6_px_in => sig6_px, chi6_py_in => sig6_py, chi6_v_in => sig6_v, chi6_theta_in => sig6_th, chi6_omega_in => sig6_om, chi6_a_in => sig6_ac, chi6_z_in => sig6_z,
      chi7_px_in => sig7_px, chi7_py_in => sig7_py, chi7_v_in => sig7_v, chi7_theta_in => sig7_th, chi7_omega_in => sig7_om, chi7_a_in => sig7_ac, chi7_z_in => sig7_z,
      chi8_px_in => sig8_px, chi8_py_in => sig8_py, chi8_v_in => sig8_v, chi8_theta_in => sig8_th, chi8_omega_in => sig8_om, chi8_a_in => sig8_ac, chi8_z_in => sig8_z,
      chi9_px_in => sig9_px, chi9_py_in => sig9_py, chi9_v_in => sig9_v, chi9_theta_in => sig9_th, chi9_omega_in => sig9_om, chi9_a_in => sig9_ac, chi9_z_in => sig9_z,
      chi10_px_in => sig10_px, chi10_py_in => sig10_py, chi10_v_in => sig10_v, chi10_theta_in => sig10_th, chi10_omega_in => sig10_om, chi10_a_in => sig10_ac, chi10_z_in => sig10_z,
      chi11_px_in => sig11_px, chi11_py_in => sig11_py, chi11_v_in => sig11_v, chi11_theta_in => sig11_th, chi11_omega_in => sig11_om, chi11_a_in => sig11_ac, chi11_z_in => sig11_z,
      chi12_px_in => sig12_px, chi12_py_in => sig12_py, chi12_v_in => sig12_v, chi12_theta_in => sig12_th, chi12_omega_in => sig12_om, chi12_a_in => sig12_ac, chi12_z_in => sig12_z,
      chi13_px_in => sig13_px, chi13_py_in => sig13_py, chi13_v_in => sig13_v, chi13_theta_in => sig13_th, chi13_omega_in => sig13_om, chi13_a_in => sig13_ac, chi13_z_in => sig13_z,
      chi14_px_in => sig14_px, chi14_py_in => sig14_py, chi14_v_in => sig14_v, chi14_theta_in => sig14_th, chi14_omega_in => sig14_om, chi14_a_in => sig14_ac, chi14_z_in => sig14_z,
      chi0_px_out => pr0_px, chi0_py_out => pr0_py, chi0_v_out => pr0_v, chi0_theta_out => pr0_th, chi0_omega_out => pr0_om, chi0_a_out => pr0_ac, chi0_z_out => pr0_z,
      chi1_px_out => pr1_px, chi1_py_out => pr1_py, chi1_v_out => pr1_v, chi1_theta_out => pr1_th, chi1_omega_out => pr1_om, chi1_a_out => pr1_ac, chi1_z_out => pr1_z,
      chi2_px_out => pr2_px, chi2_py_out => pr2_py, chi2_v_out => pr2_v, chi2_theta_out => pr2_th, chi2_omega_out => pr2_om, chi2_a_out => pr2_ac, chi2_z_out => pr2_z,
      chi3_px_out => pr3_px, chi3_py_out => pr3_py, chi3_v_out => pr3_v, chi3_theta_out => pr3_th, chi3_omega_out => pr3_om, chi3_a_out => pr3_ac, chi3_z_out => pr3_z,
      chi4_px_out => pr4_px, chi4_py_out => pr4_py, chi4_v_out => pr4_v, chi4_theta_out => pr4_th, chi4_omega_out => pr4_om, chi4_a_out => pr4_ac, chi4_z_out => pr4_z,
      chi5_px_out => pr5_px, chi5_py_out => pr5_py, chi5_v_out => pr5_v, chi5_theta_out => pr5_th, chi5_omega_out => pr5_om, chi5_a_out => pr5_ac, chi5_z_out => pr5_z,
      chi6_px_out => pr6_px, chi6_py_out => pr6_py, chi6_v_out => pr6_v, chi6_theta_out => pr6_th, chi6_omega_out => pr6_om, chi6_a_out => pr6_ac, chi6_z_out => pr6_z,
      chi7_px_out => pr7_px, chi7_py_out => pr7_py, chi7_v_out => pr7_v, chi7_theta_out => pr7_th, chi7_omega_out => pr7_om, chi7_a_out => pr7_ac, chi7_z_out => pr7_z,
      chi8_px_out => pr8_px, chi8_py_out => pr8_py, chi8_v_out => pr8_v, chi8_theta_out => pr8_th, chi8_omega_out => pr8_om, chi8_a_out => pr8_ac, chi8_z_out => pr8_z,
      chi9_px_out => pr9_px, chi9_py_out => pr9_py, chi9_v_out => pr9_v, chi9_theta_out => pr9_th, chi9_omega_out => pr9_om, chi9_a_out => pr9_ac, chi9_z_out => pr9_z,
      chi10_px_out => pr10_px, chi10_py_out => pr10_py, chi10_v_out => pr10_v, chi10_theta_out => pr10_th, chi10_omega_out => pr10_om, chi10_a_out => pr10_ac, chi10_z_out => pr10_z,
      chi11_px_out => pr11_px, chi11_py_out => pr11_py, chi11_v_out => pr11_v, chi11_theta_out => pr11_th, chi11_omega_out => pr11_om, chi11_a_out => pr11_ac, chi11_z_out => pr11_z,
      chi12_px_out => pr12_px, chi12_py_out => pr12_py, chi12_v_out => pr12_v, chi12_theta_out => pr12_th, chi12_omega_out => pr12_om, chi12_a_out => pr12_ac, chi12_z_out => pr12_z,
      chi13_px_out => pr13_px, chi13_py_out => pr13_py, chi13_v_out => pr13_v, chi13_theta_out => pr13_th, chi13_omega_out => pr13_om, chi13_a_out => pr13_ac, chi13_z_out => pr13_z,
      chi14_px_out => pr14_px, chi14_py_out => pr14_py, chi14_v_out => pr14_v, chi14_theta_out => pr14_th, chi14_omega_out => pr14_om, chi14_a_out => pr14_ac, chi14_z_out => pr14_z,
      done => prop_done
    );

  mean_inst : predicted_mean_7d
    port map (
      clk => clk, start => mean_start,
      chi0_s1 => pr0_px, chi0_s2 => pr0_py, chi0_s3 => pr0_v, chi0_s4 => pr0_th, chi0_s5 => pr0_om, chi0_s6 => pr0_ac, chi0_s7 => pr0_z,
      chi1_s1 => pr1_px, chi1_s2 => pr1_py, chi1_s3 => pr1_v, chi1_s4 => pr1_th, chi1_s5 => pr1_om, chi1_s6 => pr1_ac, chi1_s7 => pr1_z,
      chi2_s1 => pr2_px, chi2_s2 => pr2_py, chi2_s3 => pr2_v, chi2_s4 => pr2_th, chi2_s5 => pr2_om, chi2_s6 => pr2_ac, chi2_s7 => pr2_z,
      chi3_s1 => pr3_px, chi3_s2 => pr3_py, chi3_s3 => pr3_v, chi3_s4 => pr3_th, chi3_s5 => pr3_om, chi3_s6 => pr3_ac, chi3_s7 => pr3_z,
      chi4_s1 => pr4_px, chi4_s2 => pr4_py, chi4_s3 => pr4_v, chi4_s4 => pr4_th, chi4_s5 => pr4_om, chi4_s6 => pr4_ac, chi4_s7 => pr4_z,
      chi5_s1 => pr5_px, chi5_s2 => pr5_py, chi5_s3 => pr5_v, chi5_s4 => pr5_th, chi5_s5 => pr5_om, chi5_s6 => pr5_ac, chi5_s7 => pr5_z,
      chi6_s1 => pr6_px, chi6_s2 => pr6_py, chi6_s3 => pr6_v, chi6_s4 => pr6_th, chi6_s5 => pr6_om, chi6_s6 => pr6_ac, chi6_s7 => pr6_z,
      chi7_s1 => pr7_px, chi7_s2 => pr7_py, chi7_s3 => pr7_v, chi7_s4 => pr7_th, chi7_s5 => pr7_om, chi7_s6 => pr7_ac, chi7_s7 => pr7_z,
      chi8_s1 => pr8_px, chi8_s2 => pr8_py, chi8_s3 => pr8_v, chi8_s4 => pr8_th, chi8_s5 => pr8_om, chi8_s6 => pr8_ac, chi8_s7 => pr8_z,
      chi9_s1 => pr9_px, chi9_s2 => pr9_py, chi9_s3 => pr9_v, chi9_s4 => pr9_th, chi9_s5 => pr9_om, chi9_s6 => pr9_ac, chi9_s7 => pr9_z,
      chi10_s1 => pr10_px, chi10_s2 => pr10_py, chi10_s3 => pr10_v, chi10_s4 => pr10_th, chi10_s5 => pr10_om, chi10_s6 => pr10_ac, chi10_s7 => pr10_z,
      chi11_s1 => pr11_px, chi11_s2 => pr11_py, chi11_s3 => pr11_v, chi11_s4 => pr11_th, chi11_s5 => pr11_om, chi11_s6 => pr11_ac, chi11_s7 => pr11_z,
      chi12_s1 => pr12_px, chi12_s2 => pr12_py, chi12_s3 => pr12_v, chi12_s4 => pr12_th, chi12_s5 => pr12_om, chi12_s6 => pr12_ac, chi12_s7 => pr12_z,
      chi13_s1 => pr13_px, chi13_s2 => pr13_py, chi13_s3 => pr13_v, chi13_s4 => pr13_th, chi13_s5 => pr13_om, chi13_s6 => pr13_ac, chi13_s7 => pr13_z,
      chi14_s1 => pr14_px, chi14_s2 => pr14_py, chi14_s3 => pr14_v, chi14_s4 => pr14_th, chi14_s5 => pr14_om, chi14_s6 => pr14_ac, chi14_s7 => pr14_z,
      mean_s1 => xp1, mean_s2 => xp2, mean_s3 => xp3, mean_s4 => xp4,
      mean_s5 => xp5, mean_s6 => xp6, mean_s7 => xp7,
      done => mean_done
    );
      ...
      if reset = '1' then
        ...
        case fsm_state is
          ...
          when WAIT_MEAN =>
            ...
            if mean_done = '1' and pcov_done = '1' then
          ...
          when WAIT_INNOV_COV =>
            ...
            if ic_done = '1' and inn_done = '1' and cc_done = '1' then
              kg_start <= '1';
          ...
          when LATCH_OUTPUT =>
            ...
            t_state <= xu4; w_state <= xu5; a_state <= xu6; z_state <= xu7;
            ...
            theta_current <= xu4; omega_current <= xu5; a_current <= xu6; z_current <= xu7;
