...
architecture Behavioral of state_update_7d is
  ...
  constant R11 : signed(47 downto 0) := to_signed(33554432, 48);
  constant R22 : signed(47 downto 0) := to_signed(33554432, 48);
  constant R33 : signed(47 downto 0) := to_signed(33554432, 48);
    ...
    if rising_edge(clk) then
      case state is
        ...
        when COMPUTE_KR =>
          ...
          for r in 1 to 7 loop
            kr(r,1) <= resize(kr11, 96) when r = 1 else
                       resize(kr21, 96) when r = 2 else
                       resize(kr31, 96) when r = 3 else
                       resize(kr41, 96) when r = 4 else
                       resize(kr51, 96) when r = 5 else
                       resize(kr61, 96) when r = 6 else
                       resize(kr71, 96);
          end loop;
        ...
        when COMPUTE_KRK =>
          ...
          for r in 1 to 7 loop
            for c in r to 7 loop
              ...
              krk_sum := krk_sum + kr(r,1) * (
                kr11 when c=1 else kr21 when c=2 else kr31 when c=3 else
                kr41 when c=4 else kr51 when c=5 else kr61 when c=6 else kr71);

              krk_sum := krk_sum + kr(r,2) * (
                kr12 when c=1 else kr22 when c=2 else kr32 when c=3 else
                kr42 when c=4 else kr52 when c=5 else kr62 when c=6 else kr72);

              krk_sum := krk_sum + kr(r,3) * (
                kr13 when c=1 else kr23 when c=2 else kr33 when c=3 else
                kr43 when c=4 else kr53 when c=5 else kr63 when c=6 else kr73);
              krk_mat(r,c) <= krk_sum;
              if r /= c then
                krk_mat(c,r) <= krk_sum;
              end if;
