        ...
        if rising_edge(clk) then
            case state is
                ...
                when IDLE =>
                    ...
                    if start = '1' then
                        report "CROSS_COV: IDLE->COMPUTE_DELTAS (latching inputs, values hex-suppressed)";
                ...
                when COMPUTE_DELTAS =>
                    report "COV_RECON: COMPUTE_DELTA idx=" & integer'image(accumulate_idx) & " (values hex-suppressed)";
                ...
                when COMPUTE_OUTER =>
                    ...
                    if accumulate_idx = 7 then
                        report "CROSS_COV: OUTER_PRODUCT idx=7 (Z-axis sigma point, values hex-suppressed)";
                    end if;
                ...
                when FINISHED =>
                    report "CROSS_COV: FINISHED (Pxz matrix, values hex-suppressed)";
                    done <= '1';
                    if start = '0' then
                        state <= IDLE;
                    end if;
