...
entity innovation_covariance_3d is
    port (
        clk   : in  std_logic;
        start : in  std_logic;
...
end innovation_covariance_3d;

architecture Behavioral of innovation_covariance_3d is
    ...
    type meas_array is array (0 to 18) of signed(47 downto 0);
    ...
    signal acc_s11, acc_s12, acc_s22 : signed(43 downto 0) := (others => '0');
    signal acc_s13, acc_s23, acc_s33 : signed(43 downto 0) := (others => '0');
            ...
            case state is
                ...
                when IDLE =>
                    ...
                    if start = '1' then
                        ...
                        chi_z_x(15) <= chi15_z_x; chi_z_y(15) <= chi15_z_y; chi_z_z(15) <= chi15_z_z;
                        chi_z_x(16) <= chi16_z_x; chi_z_y(16) <= chi16_z_y; chi_z_z(16) <= chi16_z_z;
                        chi_z_x(17) <= chi17_z_x; chi_z_y(17) <= chi17_z_y; chi_z_z(17) <= chi17_z_z;
                        chi_z_x(18) <= chi18_z_x; chi_z_y(18) <= chi18_z_y; chi_z_z(18) <= chi18_z_z;
                ...
                when ADD =>
                    ...
                    acc_s11 <= acc_s11 + resize(shift_right(weighted_11, 2*Q), 44);
                    acc_s12 <= acc_s12 + resize(shift_right(weighted_12, 2*Q), 44);
                    acc_s22 <= acc_s22 + resize(shift_right(weighted_22, 2*Q), 44);
                    acc_s13 <= acc_s13 + resize(shift_right(weighted_13, 2*Q), 44);
                    acc_s23 <= acc_s23 + resize(shift_right(weighted_23, 2*Q), 44);
                    acc_s33 <= acc_s33 + resize(shift_right(weighted_33, 2*Q), 44);
                ...
                when ADD_R =>
                    ...
                    sum_s11 <= resize(acc_s11, 49) + resize(R11_Q24_24, 49);
                    sum_s22 <= resize(acc_s22, 49) + resize(R22_Q24_24, 49);
                    sum_s33 <= resize(acc_s33, 49) + resize(R33_Q24_24, 49);

                    sum_s12 <= resize(acc_s12, 49);
                    sum_s13 <= resize(acc_s13, 49);
                    sum_s23 <= resize(acc_s23, 49);

                    state <= SATURATE;

                when SATURATE =>

                    if sum_s11 > MAX_VALUE then
                        s11 <= MAX_VALUE;
                    elsif sum_s11 < 0 then
                        s11 <= to_signed(0, 48);
                    else
                        s11 <= resize(sum_s11, 48);
                    end if;

                    if sum_s22 > MAX_VALUE then
                        s22 <= MAX_VALUE;
                    elsif sum_s22 < 0 then
                        s22 <= to_signed(0, 48);
                    else
                        s22 <= resize(sum_s22, 48);
                    end if;

                    if sum_s33 > MAX_VALUE then
                        s33 <= MAX_VALUE;
                    elsif sum_s33 < 0 then
                        s33 <= to_signed(0, 48);
                    else
                        s33 <= resize(sum_s33, 48);
                    end if;
                ...
                when FINISHED =>
                    report "INNOVATION_COV: FINISHED (S matrix, Q24.24)" & LF &
                           "  s11 = " & integer'image(to_integer(s11)) &
                           " (" & real'image(real(to_integer(s11)) / 16777216.0) & ")" & LF &
                           "  s22 = " & integer'image(to_integer(s22)) &
                           " (" & real'image(real(to_integer(s22)) / 16777216.0) & ")" & LF &
                           "  s33 = " & integer'image(to_integer(s33)) &
                           " (" & real'image(real(to_integer(s33)) / 16777216.0) & ")" & LF &
                           "  s12 = " & integer'image(to_integer(s12)) & LF &
                           "  s13 = " & integer'image(to_integer(s13)) & LF &
                           "  s23 = " & integer'image(to_integer(s23));
                    done <= '1';
                    if start = '0' then
                        state <= IDLE;
                    end if;
